...
const CSV_FILE = path.resolve(process.env.CSV_FILE || 'imports/tilda-parisian.csv');
...
function buildFeatures(source) {
  ...
  if (lower.includes('легк')) {
    features.push('легкая конструкция');
  }

  if (lower.includes('изящн') || lower.includes('стройн')) {
    features.push('изящная форма');
  }

  if (lower.includes('состоит из двух частей') || lower.includes('двух частей')) {
    features.push('двухсоставная конструкция');
  }
  ...
  if (lower.includes('удобн') && lower.includes('использован')) {
    features.push('удобен в использовании');
  }

  if (lower.includes('толстой стали') || lower.includes('толстая сталь') || lower.includes('толст')) {
    features.push('толстая сталь');
  }

  if (lower.includes('необычн') && lower.includes('форм')) {
    features.push('необычная форма верхней части');
  }

  if (lower.includes('узор')) {
    features.push('узор верхней части');
  }
...
function buildParisianSpecifications(row, parentRow) {
...
async function main() {
  ...
  try {
    ...
    for (const row of rowsWithSku) {
      ...
      const result = buildParisianSpecifications(row, parentRow);
    ...
    if (!APPLY) {
      ...
      console.log('APPLY=1 CSV_FILE=imports/tilda-parisian.csv node scripts/import-tilda-parisian-specs.js');
      return;
    }
